...
static int __init rnddev_init(void)
{
    ...
    if (crs_len == 0)
    {
        printk(KERN_ALERT "EINVAL: crs_len is zero! Cannot make a sequence. Abort.\n");
        return -EINVAL;
    }
...
static ssize_t rnddev_read(struct file *filp,
                           char *buffer,
                           size_t length,
                           loff_t *offset)
{
...
static ssize_t rnddev_write(struct file *filp,
                            const char *buff,
                            size_t length,
                            loff_t *offset)
{
